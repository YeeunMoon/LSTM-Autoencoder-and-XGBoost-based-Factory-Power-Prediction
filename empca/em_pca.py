...
def _solve(A, b, w):
    ...
    # Explicitly specifying rcond=None to use the future default behavior
    if isinstance(A, scipy.sparse.spmatrix):
        x = scipy.sparse.linalg.spsolve(A, b)
    else:
        x = np.linalg.lstsq(A, b, rcond=None)[0]  # Set rcond=None for future behavior

    return x

#-------------------------------------------------------------------------

def em_pca(data, weights=None, niter=25, nvec=5, smooth=0, randseed=1, silent=False):
    """
    ...
    for k in range(niter):
        ...
        if not silent:
            print('em_pca %2d/%2d  %15.8f %15.8f' % \
...
def _main():
    ...
    print("Testing empca")
    m0 = em_pca(noisy_data, weights, niter=20)
